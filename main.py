...
def update_graph():
    ...
    def animate_graph():
        ...
        if (selected_algorithm not in ['bfs', 'dfs','bms']):
            val = val[1:]
        start = val[0]
        for node in val[1:]:
            end = node
...
def algo_select(algo):
    ...
    if algo == "bfs":
        ...
        document["paragh"].innerText = "Selected British Museum search"
    elif algo == "hc":
        ...
        document["paragh"].innerText = "Selected Branch and bound + Extended List"
    elif algo == "bb-h":
...
document['bms'].bind('click', lambda e: algo_select('bms'))
